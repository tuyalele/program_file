...
//    #error "请在此处理可下发可上报数据及只上报数据示例,处理完成后删除该行"
    ...
    mcu_dp_value_update(DPID_HUMIDITY_VALUE,humidity); //VALUE型数据上报;当前当前湿度
    mcu_dp_value_update(DPID_TEMP_SAMPLING,CY_T_time); //VALUE型数据上报;当前温度采样时间设置
    mcu_dp_value_update(DPID_HUMIDITY_SAMPLING,CY_H_time); //VALUE型数据上报;当前湿度采样时间设置
    mcu_dp_value_update(DPID_MAXTEMP_SET,most_T); //VALUE型数据上报;当前温度上限设置
    mcu_dp_value_update(DPID_MINITEMP_SET,least_T); //VALUE型数据上报;当前温度下限设置
    mcu_dp_value_update(DPID_MAXHUM_SET,most_humidity); //VALUE型数据上报;当前湿度上限设置
    mcu_dp_value_update(DPID_MINIHUM_SET,least_humidity); //VALUE型数据上报;当前湿度下限设置
    mcu_dp_enum_update(DPID_TEMP_ALARM,JB_T); //枚举型数据上报;当前温度报警
    mcu_dp_enum_update(DPID_HUM_ALARM,JB_humidity); //枚举型数据上报;当前湿度报警
    mcu_dp_value_update(DPID_BRIGHT_VALUE,sun); //VALUE型数据上报;
    mcu_dp_string_update(DPID_DATA_IS,&DATA[0],DATA_long); //STRING型数据上报;当前DATA指针,当前DATA数据长度
    ...
    mcu_dp_string_update(DPID_DATA_IS,当前DATA指针,当前DATA数据长度); //STRING型数据上报;
...
/*****************************************************************************
函数名称 : dp_download_data_is_handle
功能描述 : 针对DPID_DATA_IS的处理函数
输入参数 : value:数据源数据
        : length:数据长度
返回参数 : 成功返回:SUCCESS/失败返回:ERROR
使用说明 : 可下发可上报类型,需要在处理完数据后上报处理结果至app
*****************************************************************************/
static unsigned char dp_download_data_is_handle(const unsigned char value[], unsigned short length)
{
    //示例:当前DP类型为STRING
    unsigned char ret;
    /*
    ...
    */
    ...
    //处理完DP数据后应有反馈
    ret = mcu_dp_string_update(DPID_DATA_IS,value, length);
    if(ret == SUCCESS)
        return SUCCESS;
    else
        return ERROR;
...
unsigned char dp_download_handle(unsigned char dpid,const unsigned char value[], unsigned short length)
{
    ...
    ***********************************/
    ...
    switch(dpid) {
        ...
        case DPID_DATA_IS:
            //DATA处理函数
            ret = dp_download_data_is_handle(value,length);
        break;
...
//					PrintString1("未扫描到名称为tuya_mdev_test路由器,请检查!\r\n");	//SUART1发送一个字符串
...
//			PrintN1(rssi);//发数值
			OLED_ShowString(17,2,"rssi:",12);
			OLED_ShowNum(17,4,rssi,2,16);
    }
}
#endif
